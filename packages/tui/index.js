...
// Join every text block (nested included). Tool results nest their payload
// inside a tool-result block, so a flat scan of `message.content` misses it.
...
function setupUi(runtimeRef, modelInfo) {
  ...
  function buildHelp() {
    ...
    return [
      ...
      cmdRow('/theme <name>', `switch theme: ${themeNames.join(', ')}`),
