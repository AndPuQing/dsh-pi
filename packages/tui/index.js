...
import { createWebSurface } from '@dsh-pi/web'
...
// `dsh-pi tui --serve [port]` also mounts the web surface on the SAME runtime:
// the browser becomes a second renderer of this session (live push over SSE —
// no polling — and it can send prompts, switch sessions, rename, interrupt;
// this process stays the session's single live writer, so the two surfaces
// never contend for ownership).
function parseServeArg(argv) {
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a.startsWith('--serve=')) {
      ...
      return Number.isFinite(n) && n > 0 ? n : 8123
    }
    if (a === '--help') {
      console.log('usage: dsh-pi-tui [--serve [port]]\n\n--serve [port]  also serve the web surface (default port 8123)')
      process.exit(0)
    }
  }
  return null
...
async function main() {
  ...
  const servePort = parseServeArg(process.argv.slice(2))

  // UI first: instant; boot the runtime in the background
  const runtimeRef = { ready: false, sessionId: null, prompt: null, interrupt: null, newSession: null, listSessions: null, switchSession: null, deleteSession: null, forkSession: null, renameSession: null, currentTitle: null, conversationHistory: null, readImage: null, dispose: null, onEvent: null, refreshStatus: null, closeWeb: null, setModel: null, listModels: null, exportSession: null, compact: null }
  ...
  ;(async () => {
    try {
      const runtime = await createPiRuntime({
        onSubagent: (n) => ui.setSubagents(n),
        onSwitched: () => ui.onSessionSwitched(),
      })
      ...
      // live getter — the status line always shows the CURRENT session even
      // after a switch from this surface or the web surface
      Object.defineProperty(runtimeRef, 'sessionId', { configurable: true, get: () => runtime.sessionId })
      ...
      if (servePort) {
        try {
          const web = await createWebSurface(runtime, { host: '127.0.0.1', port: servePort, name: 'dsh-pi tui' })
          runtimeRef.closeWeb = web.close
          ui.addMessage('sys', `web surface: ${web.url} — 浏览器实时查看/操作同一会话`)
        } catch (e) {
          ui.addMessage('error', `web surface: ${e.message}`)
        }
      }
      ui.setReady()
    } catch (e) {
...
export { cycleModelSelection, resolveModelArg, formatTokens, shortenPath, pickImageMime, recentSessions }
// content/session helpers live in @dsh-pi/runtime (shared with the web
// surface) — re-exported here for the TUI's public API and its tests
export { collectImageBlocks, contentText, reasoningBlocks, reasoningSummary, toolCallInfo, toolResultMessage, sessionTitleFromEvents, exportSessionMarkdown, exportSessionJson, exportFileName } from '@dsh-pi/runtime'
